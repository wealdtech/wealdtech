...
/**
 * Create object mappers, with an option to create
 * non-standard mappers if required.
 * The default object mapper contains defaults suitable
 * for most real-world usage, and additional serializers
 * and deserializers for common types not supported natively
 * by Jackson.  Additional features can be configured using
...
//    DEFAULTMAPPER.enable(DeserializationFeature.ACCEPT_EMPTY_STRING_AS_NULL_OBJECT);
   ...
   * @param configuration the objectmapper configuration
...
//    SERVERMAPPER.enable(DeserializationFeature.ACCEPT_EMPTY_STRING_AS_NULL_OBJECT);
    ...
    final InjectableValues serverinject = new InjectableValues.Std().addValue("AllowPartials", Boolean.TRUE);
