 ...
 */
@JsonTypeName("google")
public class GoogleHandle extends SiteHandle<GoogleHandle> implements Comparable<GoogleHandle>
{
  private static final String _TYPE = "google";

  private static final Pattern ACCOUNT_URL = Pattern.compile("^https?://(www\\.)?google\\.com/profiles/([^/?]+)");
  private static final Pattern PLUS_URL = Pattern.compile("^https?://plus\\.google\\.com/([^/?]+)");
  ...
  {
    return url != null && (ACCOUNT_URL.matcher(url).matches() || PLUS_URL.matcher(url).matches());
  }
  ...
  public static String localIdFromAccountUrl(@Nullable final String url)
  {
    ...
    final Matcher plusMatcher = PLUS_URL.matcher(url);
    if (plusMatcher.find())
    {
  ...
  public static class Builder<P extends Builder<P>> extends SiteHandle.Builder<GoogleHandle, P>
  {
    ...
    public Builder(final GoogleHandle prior)
    {
      super(prior);
    }

    public GoogleHandle build()
    {
      return new GoogleHandle(data);
    }
  }
  ...
  public static Builder<?> builder(final GoogleHandle prior)
  {
