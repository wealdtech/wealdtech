...
public interface ContactsSyncService<C extends Credentials>
{
   ...
   * @param full carry out a full import by removing any items in the destination that are not in the source
   * @return the number of contacts imported
   */
  int importContacts(C credentials, boolean full);

  /**
   * Export contacts to the given source
   * @param full carry out a full export by removing any items in the destination that are not in the source
   * @return the number of contacts exported
   */
  int exportContacts(C credentials, boolean full);
